...
} = {}) => props.reduce((p, prop) => ({ ...p,
  [`${prop}${suffix}`]: {
    get() {
      return getter ? getter.call(this, prop) : getFormatter(this[prop]);
    },

    set(val) {
      const event = prop === 'value' ? 'input' : `update:${prop}`;
      setter ? setter.call(this, prop, val) : this.$emit(event, setFormatter(val));
    }

  }
}), {});

var _default = createPropProxy;
